...
public class SerializableHttpCookie implements Serializable {
    ...
    private static final long serialVersionUID = 6374381323722046733L;
    ...
    public HttpCookie decode(String str) throws NoSuchFieldException, ClassNotFoundException, IOException {
        try {
            Object object = new ObjectInputStream(new ByteArrayInputStream(HashUtils.hexStringToByteArray(str))).readObject();
            if (object instanceof com.synology.sylib.syhttp.cookieStore.SerializableHttpCookie) {
                object = changePackage((com.synology.sylib.syhttp.cookieStore.SerializableHttpCookie) object);
            }
            return ((SerializableHttpCookie) object).cookie;
        } catch (IOException e) {
    ...
    private static SerializableHttpCookie changePackage(com.synology.sylib.syhttp.cookieStore.SerializableHttpCookie serializableHttpCookie) throws NoSuchFieldException {
        try {
            Field declaredField = serializableHttpCookie.getClass().getDeclaredField("cookie");
            declaredField.setAccessible(true);
            return new SerializableHttpCookie((HttpCookie) declaredField.get(serializableHttpCookie));
